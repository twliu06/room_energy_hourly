...
TABLE_NAME = "sensor.room_504_energy_hourly"
TABLE_COMMENT = "科航504 教室每小時用電量資料（來源：SQL Server View_KWH_hr_504）"

# (欄位名稱, 型別, NOT NULL, 欄位註解)
COLUMNS = [
    ("id", "BIGSERIAL PRIMARY KEY", True, "資料識別碼"),

    ("DATE_504", "TEXT", False, "用電日期 (原始)"),
    ("DATE_504_H", "SMALLINT", False, "用電小時 (原始)"),

    ("Elc_IP_KWH_Hr", "TEXT", False, "插座總用電 (原始)"),

    ("Air_A_KWH_Hr", "TEXT", False, "空調A (原始)"),
    ("Air_B_KWH_Hr", "TEXT", False, "空調B (原始)"),
    ("Air_C_KWH_Hr", "TEXT", False, "空調C (原始)"),

    ("created_at", "TIMESTAMP(6)", True, "資料建立時間"),
    ("updated_at", "TIMESTAMP(6)", True, "資料更新時間"),
]

UNIQUE_KEYS = ["DATE_504", "DATE_504_H"]
